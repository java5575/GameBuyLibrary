...
public class RDBProductsDAO {
    ...
    private static final String SELECT_BY_NAME_SQL = "SELECT id, " + COL_LIST + " FROM products"
            + " WHERE name like ?";
...
//                        p.setStock(rs.getInt("stock"));
                        ...
                        String platForm = rs.getString("platform.chname");
                        p.setPlatForm(PlatForm.convertFromChname(platForm));

                        String gameType = rs.getString("gametype.chname");
                        p.setGameType(GameType.convertFromChname(gameType));
                        list.add(p);
                    } catch (RuntimeException ex) {
                        System.out.println("產品資料讀取錯誤: " + ex);
                    }
    ...
    public List<Product> getPlatForm(int gametype_idgametype) throws GameBuyException {
        List<Product> list = new ArrayList<>();
        try (Connection connection = RDBConnection.getConnection();
                PreparedStatement pstmt = connection.prepareStatement(JOIN_SELECT_PLATFROM_SQL);) {
            pstmt.setInt(1, gametype_idgametype);//傳SQL指令
            try (ResultSet rs = pstmt.executeQuery();) {
                while (rs.next()) {
                    Product p = new Product();
...
//                        p.setStock(rs.getInt("stock"));
        ...
        List<Product> list = new ArrayList<>();
        try (Connection connection = RDBConnection.getConnection();
                PreparedStatement pstmt = connection.prepareStatement(SELECT_BY_NAME_SQL);) {
...
//            System.out.println("productList = " + list);
            ...
            throw new GameBuyException("查詢產品名稱失敗", ex);
        }
    }
